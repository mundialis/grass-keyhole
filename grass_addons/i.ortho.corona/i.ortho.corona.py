...
def main():
    ...
    if not grass.find_program("i.ortho.position", "--help"):
        grass.fatal(
            _("The 'i.ortho.position' module was not found, install it first:")
            + "\n"
    ...
    rastername_keys = list(
        grass.parse_command("i.group", group=group, flags="lg").keys()
    )
    ...
    # get fiducials
    fid_nw, fid_ne, fid_se, fid_sw = getfiducials(
        int(raster_rows), int(raster_cols), scan_resolution
    )

    fidstring = "%s,%s,%s,%s,%s,%s,%s,%s" % (
        fid_nw[0],
        fid_nw[1],
        fid_ne[0],
        fid_ne[1],
    ...
    ref_points_string = string_refpoints(
        fid_nw, fid_ne, fid_se, fid_sw, raster_rows, raster_cols
    )
    ref_points_path = "%s/%s/%s/group/%s/REF_POINTS" % (
        start_gisdbase,
        start_location,
        start_mapset,
        group,
    ...
    control_points_path = "%s/%s/%s/group/%s/CONTROL_POINTS" % (
        start_gisdbase,
        start_location,
        start_mapset,
        group,
    ...
    if not transform_only:
        if options["logfile"]:
            ...
            stdout_stderr = grass.Popen(
                "i.ortho.transform -sp group=%s --v" % group,
                shell=True,
            ...
            if os.path.isfile(logfile):
                grass.warning(
                    _("File %s already exists, will be overwritten..." % logfile)
                )
